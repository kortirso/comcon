RSpec.describe ProfessionForm, type: :service do
  describe '.persist?' do
    context 'for invalid data' do
      let(:service) { ProfessionForm.new(name: { 'en' => '', 'ru' => '' }) }

      it 'does not create new profession' do
        expect { service.persist? }.to_not change(Profession, :count)
      end
    ...
    context 'for valid data' do
      ...
      context 'for existed profession' do
        let(:service) { ProfessionForm.new(name: { 'en' => profession.name['en'], 'ru' => profession.name['ru'] }) }
      ...
      context 'for unexisted profession' do
        let(:service) { ProfessionForm.new(name: { 'en' => 'Skinning', 'ru' => 'Снятие шкур' }) }
