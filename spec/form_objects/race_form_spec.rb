RSpec.describe RaceForm, type: :service do
  describe '.persist?' do
    context 'for invalid data' do
      let(:service) { RaceForm.new(name: { 'en' => '', 'ru' => '' }) }

      it 'does not create new race' do
        expect { service.persist? }.to_not change(Race, :count)
      end
    ...
    context 'for valid data' do
      ...
      context 'for existed race' do
        let(:service) { RaceForm.new(name: { 'en' => race.name['en'], 'ru' => race.name['ru'] }, fraction: fraction) }
      ...
      context 'for unexisted race' do
        let(:service) { RaceForm.new(name: { 'en' => 'Human', 'ru' => 'Человек' }, fraction: fraction) }
