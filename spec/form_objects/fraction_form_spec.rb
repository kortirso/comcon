RSpec.describe FractionForm, type: :service do
  describe '.persist?' do
    context 'for invalid data' do
      let(:service) { FractionForm.new(name: { 'en' => '', 'ru' => '' }) }

      it 'does not create new fraction' do
        expect { service.persist? }.to_not change(Fraction, :count)
      end
    ...
    context 'for valid data' do
      ...
      context 'for existed fraction' do
        let(:service) { FractionForm.new(name: { 'en' => fraction.name['en'], 'ru' => fraction.name['ru'] }) }
      ...
      context 'for unexisted fraction' do
        let(:service) { FractionForm.new(name: { 'en' => 'Alliance', 'ru' => 'Альянс' }) }
