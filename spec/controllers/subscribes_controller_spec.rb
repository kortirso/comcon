RSpec.describe SubscribesController, type: :controller do
  ...
  describe 'POST#approve' do
    ...
    context 'for logged user' do
      ...
      context 'for invalid params' do
        let(:request) { post :approve, params: { locale: 'en', subscribe: { event_id: nil } } }
      ...
      context 'for valid params' do
        let(:request) { post :approve, params: { locale: 'en', subscribe: { event_id: event.id, character_id: character.id } } }

        it 'creates new subscribe' do
          expect { request }.to change { character.subscribes.count }.by(1)
        end

        it 'and its status is approved' do
          request

          expect(Subscribe.last.status).to eq 'approved'
        end
    ...
    def do_request
      post :approve, params: { locale: 'en', subscribe: { event_id: event.id, character_id: nil } }
    end
  end
end
