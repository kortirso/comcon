...
describe CreateCharacterRoles do
  ...
  describe '.call' do
    context 'for unexisted character role' do
      let(:interactor) { described_class.call(character_id: character.id, character_role_params: { 'roles' => { role.id.to_s => '1' } }) }
    ...
    context 'for unexisted character role, with main' do
      let(:interactor) { described_class.call(character_id: character.id, character_role_params: { 'main_role_id' => role.id.to_s, 'roles' => { role.id.to_s => '1' } }) }
    ...
    context 'for existed character role' do
      ...
      let(:interactor) { described_class.call(character_id: character.id, character_role_params: { 'roles' => { role.id.to_s => '0' } }) }
