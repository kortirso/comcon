...
class CheckAddedHeadRole
  ...
  def call
    ...
    return unless Delivery.exists?(deliveriable: context.guild_role.guild, notification: guild_notification)

    user_notification = Notification.find_by(event: 'guild_request_creation', status: 1)
    return if user_notification.nil?
    return if Delivery.exists?(deliveriable: user, notification: user_notification)
