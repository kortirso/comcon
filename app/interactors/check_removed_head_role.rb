...
class CheckRemovedHeadRole
  ...
  def call
    ...
    other_head_characters_of_user_in_guild = user.characters.where.not(id: context.guild_role.character_id).where(guild_id: context.guild_role.guild_id).joins(:guild_role).where(guild_roles: { name: 'gm' })
